...
input_parser() {
    ...
    opts_AddOptional  '--time' 'time' 'limit on the total run time of the job allocation' "an optional value; When the time limit is reached, each task in each job step is sent SIGTERM followed by SIGKILL. Format: days-hours:minutes:seconds. Default 4 hours: None" "0-04:00:00"
    ...
    opts_AddOptional  '--mem-per-cpu' 'memPerCPU' 'minimum memory allocated memory per CPU' "an optional value; specify the minimum real memory required per CPU. Default: 16gb" "16gb"
    opts_AddOptional  '--mem' 'mem' 'specify the real memory requried per node' "an optional value; specify the real memory required per node. Default: 16gb" "16gb"
